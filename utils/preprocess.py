...
class Preprocessor:
    ...
    def _simulate_text_line(self, batch: Batch) -> Batch:
        ...
        for i in range(batch.batch_size):
            ...
            num_words = random.randint(1, 16) if self.data_augmentation else default_num_words
...
"""def main():
    ...
    img = cv2.imread('../Test/a.jpg', cv2.IMREAD_GRAYSCALE)
    img_aug = []
    for i in range(9):
        img_aug.append(Preprocessor((256, 64), data_augmentation=True).process_img(img))
    fig, axs = plt.subplots(3, 3)

    axs[0,0].imshow(img, cmap='gray')
    axs[0,1].imshow(cv2.transpose(img_aug[0]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[0, 2].imshow(cv2.transpose(img_aug[1]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[1, 0].imshow(cv2.transpose(img_aug[2]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[1, 1].imshow(cv2.transpose(img_aug[3]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[1, 2].imshow(cv2.transpose(img_aug[4]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[2, 0].imshow(cv2.transpose(img_aug[5]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[2, 1].imshow(cv2.transpose(img_aug[6]) + 0.5, cmap='gray', vmin=0, vmax=1)
    axs[2, 2].imshow(cv2.transpose(img_aug[7]) + 0.5, cmap='gray', vmin=0, vmax=1)
    for ax in axs.flat:
        ax.axis('off')
    plt.show()
